...
export class NameValidator {
  ...
  private validateBasicFormat(name: string, result: NameValidationResult, type: string): void {
    ...
    if (!/^[a-zA-ZáéíóúñÁÉÍÓÚÑ]/.test(name)) {
      result.isValid = false;
      result.errors.push(
        `${type === 'firstName' ? 'El nombre' : 'El apellido'} debe comenzar con una letra`,
      );
    }
    ...
    if (/[\s-']$/.test(name)) {
      result.isValid = false;
      result.errors.push(
        `${type === 'firstName' ? 'El nombre' : 'El apellido'} no puede terminar con espacio, guión o apóstrofe`,
      );
    }
  ...
  private validateCharacters(name: string, result: NameValidationResult): void {
    ...
    if (!allowedPattern.test(name)) {
      result.isValid = false;
      result.errors.push(
        'Contiene caracteres no permitidos. Solo se permiten letras, espacios' +
  ...
  private validateConsonantSequences(name: string, result: NameValidationResult): void {
    ...
    const isException = KNOWN_CONSONANT_EXCEPTIONS.some(
      (exception) => nameForCheck.includes(exception) || exception.includes(nameForCheck),
    );

    if (isException) {
      result.warnings.push(
        'Este nombre tiene una secuencia de consonantes inusual pero es un nombre conocido',
      );
      return;
    }
    ...
    if (maxConsonantSequence === this.options.consonantLimit) {
      result.warnings.push(
        `Secuencia de consonantes en el límite permitido (${this.options.consonantLimit})`,
      );
    }
  }
  ...
  private validateVowelSequences(name: string, result: NameValidationResult): void {
    ...
    if (maxVowelSequence === this.options.vowelLimit) {
      result.warnings.push(
        `Secuencia de vocales en el límite permitido (${this.options.vowelLimit})`,
      );
    }
  }
  ...
  private validateFirstName(name: string, result: NameValidationResult): void {
    ...
    if (!this.options.allowMiddleNames && /\s/.test(name)) {
      result.warnings.push(
        'Este nombre contiene espacios. Considera usar el campo de segundo nombre si está disponible',
      );
    }
  ...
  const testCases = [
    {
      ...
      shouldPass: true,
      note: 'Solo 2 consonantes consecutivas (dw)',
    },
    {
      name: 'Christian',
      type: 'firstName' as const,
      shouldPass: true,
      note: 'Excepción conocida (4 consonantes)',
    },
    {
      name: 'María José',
      type: 'firstName' as const,
      shouldPass: true,
      note: 'Nombre compuesto válido',
    },
    {
      name: 'Xxxyyy',
      type: 'firstName' as const,
      shouldPass: false,
      note: '4 consonantes consecutivas',
    },
    {
      name: 'Aeiou',
      type: 'firstName' as const,
      shouldPass: false,
      note: '5 vocales consecutivas',
    },
    {
      name: 'José-Luis',
      type: 'firstName' as const,
      shouldPass: true,
      note: 'Nombre con guión válido',
    },
    {
      name: "O'Connor",
      type: 'lastName' as const,
      shouldPass: true,
      note: 'Apellido con apóstrofe válido',
    },
    {
      name: 'García',
      type: 'lastName' as const,
      shouldPass: true,
      note: 'Apellido con acento válido',
    },
