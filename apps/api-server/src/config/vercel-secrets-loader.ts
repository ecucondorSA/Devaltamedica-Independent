...
export async function initSecrets() {
  logger.info('[secrets-loader] 🚀 Inicializando secrets...');
  ...
  if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
    try {
      ...
      if (kv) {
        logger.info('[secrets-loader] 📡 Conectando a Vercel KV...');
        ...
        if (secrets) {
          ...
          logger.info('[secrets-loader] ✅ Secrets cargados desde Vercel KV');
        } else {
      ...
      logger.warn('[secrets-loader] ⚠️ Vercel KV no disponible, usando secrets locales');
    }
  } else {
    logger.info('[secrets-loader] 📁 Usando secrets locales desde .env');
  }

  // Verificar que tengamos los secrets críticos
  if (!process.env.JWT_SECRET || !process.env.JWT_REFRESH_SECRET) {
    logger.warn('[secrets-loader] ⚠️ ADVERTENCIA: JWT secrets no encontrados');
    logger.info('[secrets-loader] 💡 Ejecuta: node scripts/setup-local-secrets.js');
  } else {
    logger.info('[secrets-loader] ✅ Secrets inicializados correctamente');
  }
}
