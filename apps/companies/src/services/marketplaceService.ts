...
export const createUrgentJobPosting = async (jobData: AutoGeneratedJobPosting) => {
  try {
    logger.info('🚀 Creating urgent job posting in marketplace:', jobData);
    ...
    logger.info('✅ Urgent job posting created successfully:', response.data.id);
    ...
    logger.error('❌ Failed to create urgent job posting:', error);
...
export const syncPendingJobPostings = async () => {
  ...
  for (const listing of pendingListings) {
    try {
      ...
      if (result.success) {
        synced++;
        logger.info(`✅ Synced pending job posting: ${listing.id}`);
      } else {
        ...
        if (listing.retry_count < 3) {
          ...
          failed++;
          logger.warn(`❌ Gave up on job posting after 3 retries: ${listing.id}`);
        }
      }
    } catch (error) {
      listing.retry_count = (listing.retry_count || 0) + 1;
