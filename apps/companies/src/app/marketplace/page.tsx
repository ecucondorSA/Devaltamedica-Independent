...
// Stubs temporales para marketplace hooks
const useCompanyProfile = () => ({ company: null, isLoading: false, error: null });
const useDoctorSearch = () => ({ doctors: [], isSearching: false, search: () => {} });
const useJobApplications = () => ({
  applications: [],
  isLoading: false,
  submitApplication: () => Promise.resolve(),
});
const useMarketplaceJobs = () => ({
  jobs: [],
  isLoading: false,
...
const mockCompanyJobs: JobPosting[] = [
  {
    ...
    urgent: true,
    description:
      'Buscamos cardiólogo especializado en procedimientos intervencionistas para nuestro departamento de cardiología.',
    requirements: [
  ...
  },
  {
    ...
    status: 'active',
    description:
      'Oncólogo para nuestro departamento de oncología con experiencia en tratamientos innovadores.',
    requirements: [
    ...
    <div className="bg-white p-6 rounded-lg border border-neutral-200 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          ...
          {trend && <p className="text-sm text-green-600 mt-1">+{trend}% vs mes anterior</p>}
        </div>
...
export default function CompanyMarketplacePage() {
  ...
  const { company: companyProfile, isLoading: isProfileLoading } =
  ...
  const applications: any[] = Array.isArray(applicationsApi?.applications)
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'jobs' | 'map' | 'analytics' | 'messages'
  >('map');
        ...
        publishedServices: [
          {
            ...
            price: { amount: 50, currency: 'USD', type: 'per_session' },
  ...
  const doctorsForMap =
    availableDoctors && (availableDoctors as any[]).length > 0
  ...
  useEffect(() => {
    if (activeTab === 'map') {
      const seen =
        typeof window !== 'undefined' ? localStorage.getItem('marketplace_onboarding_seen') : '1';
      if (!seen) {
  ...
  const rawJobs =
    Array.isArray(publishedJobs) && (publishedJobs as any[]).length > 0
  ...
  return (
    <div
      ...
      className="min-h-screen bg-vscode-editor text-vscode-foreground"
    >
      {/* Header VS Code-like */}
      <div
        data-testid="marketplace-header"
        className="sticky top-0 z-20 border-b border-vscode-border bg-vscode-activity-bar text-white"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <h1 data-testid="marketplace-title" className="text-xl font-semibold">
                Marketplace
              </h1>
              <span className="hidden md:inline text-xs opacity-80">
                Gestión integral: ofertas, mapa, analytics y mensajes
              </span>
            </div>
            <div className="flex items-center gap-2">
              <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {activeTab === 'overview' && <JobMarketplaceDashboard />}
          ...
          <div className="border border-vscode-border rounded-lg overflow-hidden h-[calc(100vh-168px)] flex bg-vscode-panel">
            ...
            {showLeftPanel && (
              <div
                ...
                className="w-[360px] border-r border-vscode-border h-full flex flex-col"
              >
                {/* Ofertas plegable */}
                <div className="border-b border-vscode-border">
                  ...
                  >
                    <span className="text-sm font-medium flex items-center gap-2">
                      <Briefcase className="w-4 h-4" /> Ofertas
                    </span>
                    <ChevronDown
                      className={`w-4 h-4 transition-transform ${openOffers ? '' : '-rotate-90'}`}
                    />
                  </button>
                  {openOffers && (
                      ...
                      {filteredJobs.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {filteredJobs.map((job) => (
                            <div
                              key={job.id}
                              className="p-3 rounded border border-vscode-border/60 bg-vscode-editor hover:bg-vscode-list-hover"
                            >
                ...
                <div className="flex-1 flex flex-col">
                  <div className="border-b border-vscode-border">
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-4 h-4 transition-transform ${openCounterparty ? '' : '-rotate-90'}`}
                      />
                    </button>
                    {openCounterparty && (
                      <div data-testid="professionals-section" className="p-3">
                        ...
                        <div
                          data-testid="professionals-list"
                          className="h-[calc(100%-56px)] overflow-y-auto -mr-2 pr-2"
                        >
                          {doctorsForMap
                            .filter(
                              (d: any) =>
                            ...
                            .map((doc: any) => (
                              ...
                              >
                                <div className="flex items-start gap-3">
                                  <div className="h-9 w-9 rounded-full bg-blue-500/20 text-blue-300 flex items-center justify-center">
                                    👨‍⚕️
                                  </div>
                                  <div className="min-w-0">
                                    ...
                                    <p className="text-[11px] opacity-70 flex items-center gap-1">
                                      <MapPin className="w-3 h-3" /> {doc.location?.city},{' '}
            ...
            {showRightPanel && (
              <div
                ...
                className="w-[360px] border-l border-vscode-border h-full flex flex-col"
              >
                ...
                <div className="border-b border-vscode-border">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-4 h-4 transition-transform ${openComms ? '' : '-rotate-90'}`}
                    />
                  </button>
                  {openComms && (
                ...
                <div className="flex-1 flex flex-col">
                  <div className="border-b border-vscode-border">
                    ...
                    >
                      <span className="text-sm font-medium flex items-center gap-2">
                        <TrendingUp className="w-4 h-4" /> Análisis Competitivo
                      </span>
                      <ChevronDown
                        className={`w-4 h-4 transition-transform ${openAnalytics ? '' : '-rotate-90'}`}
                      />
                    </button>
                    {openAnalytics && (
              ...
              {onboardingStep === 1 && (
                <div data-testid="onboarding-step-1" className="space-y-2">
                  ...
                  <p className="text-sm opacity-90">
                    Observa en el mapa el Hospital San Vicente en rojo/alerta. El sistema detecta
                    saturación de guardia/UCI y sugiere derivar.
                  </p>
                  <p className="text-sm opacity-90">
                    Esto se basa en disponibilidad, triage y reglas clínicas. No favorece
                    público/privado; prioriza tiempo a tratamiento.
                  </p>
                </div>
              )}
              {onboardingStep === 2 && (
                <div data-testid="onboarding-step-2" className="space-y-2">
                  ...
                  <p className="text-sm opacity-90">
                    Se asigna una ambulancia 🚑 según SLA y convenios. Quien paga la ambulancia
                    depende del contrato (hospital, aseguradora o coordinación regional).
                  </p>
                </div>
              )}
              {onboardingStep === 3 && (
                <div data-testid="onboarding-step-3" className="space-y-2">
                  ...
                  <p className="text-sm opacity-90">
                    Visualiza el trayecto punteado rojo desde el hospital origen al receptor. El
                    sistema estima tiempos y evita colapsos en ruta.
                  </p>
                </div>
              )}
              {onboardingStep === 4 && (
                <div data-testid="onboarding-step-4" className="space-y-2">
                  ...
                  <p className="text-sm opacity-90">
                    Se selecciona el receptor óptimo por capacidad y especialidad. Los costos
                    clínicos se rigen por convenios vigentes con el pagador.
                  </p>
                  <p className="text-sm opacity-90">
                    Como dueño del hospital, tu rol es mantener datos de disponibilidad,
                    aceptar/rechazar derivaciones por SLA y disparar derivación cuando corresponda;
                    el sistema orquesta, no decide por ti.
                  </p>
                </div>
              )}
            </div>
            <div className="p-4 border-t border-vscode-border flex items-center justify-between">
              <div data-testid="onboarding-progress" className="text-xs opacity-80">
