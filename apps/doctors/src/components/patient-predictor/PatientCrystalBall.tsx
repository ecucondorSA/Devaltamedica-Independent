...
import { usePatientPredictor } from '@altamedica/hooks/medical/usePatientPredictor';
import { 
  Card,
  CardContent,
  CardDescription,
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <Card className="border-2" style={{
        borderColor: summary.riskLevel === 'critical' ? '#ef4444' : 
                     summary.riskLevel === 'high' ? '#f97316' :
                     summary.riskLevel === 'moderate' ? '#eab308' : '#22c55e'
      }}>
        <CardHeader>
              ...
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  ...
                  <Badge variant={
                    ...
                    {summary.riskLevel === 'critical' ? 'CRÍTICO' :
                     summary.riskLevel === 'high' ? 'ALTO' :
                     summary.riskLevel === 'moderate' ? 'MODERADO' : 'BAJO'}
                  </Badge>
                </div>
              </div>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-2">
                          <Badge variant={
                            ...
                            intervention.priority === 'high' ? 'secondary' : 'default'
                          }>
                    ...
                    <div className="space-y-2">
                      {prediction.followUpPlan.schedule.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                          {item.type === 'phone_call' && <Phone className="h-4 w-4 text-blue-500" />}
                          {item.type === 'video_consultation' && <Video className="h-4 w-4 text-purple-500" />}
                          {item.type === 'in_person' && <User className="h-4 w-4 text-green-500" />}
                          {item.type === 'home_visit' && <Home className="h-4 w-4 text-orange-500" />}
                          <div className="flex-1">
                            <p className="font-medium text-sm">{item.purpose}</p>
                            <p className="text-xs text-muted-foreground">
                              {format(new Date(item.date), "d 'de' MMMM 'a las' HH:mm", { locale: es })}
                          ...
                          <Badge variant={item.status === 'scheduled' ? 'default' : 'secondary'}>
                            {item.status === 'scheduled' ? 'Programado' : 
                             ...
                             item.status === 'missed' ? 'Perdido' : 'Cancelado'}
                          </Badge>
                        </div>
                      ))}
                    </div>
                          ...
                          <CardContent className="pt-4">
                            <div className="flex items-start gap-2">
                              ...
                              <div className="space-y-1">
                                ...
                                <p className="text-xs">
                                  <span className="text-green-600">Normal: {param.normalRange}</span>
                                  {' • '}
                                  <span className="text-red-600">Crítico: {param.criticalValues}</span>
                                </p>
                              </div>
                            </div>
                          </CardContent>
                        ...
                        <>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm">
                              FC: {prediction.predictions.readmission.clinicalMarkers.vitalSigns.heartRate} lpm
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Activity className="h-4 w-4 text-blue-500" />
                            <span className="text-sm">
                              SpO2: {prediction.predictions.readmission.clinicalMarkers.vitalSigns.oxygenSaturation}%
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <ThermometerSun className="h-4 w-4 text-orange-500" />
                            <span className="text-sm">
                              Temp: {prediction.predictions.readmission.clinicalMarkers.vitalSigns.temperature}°C
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Stethoscope className="h-4 w-4 text-purple-500" />
                            <span className="text-sm">
                              PA: {prediction.predictions.readmission.clinicalMarkers.vitalSigns.bloodPressure}
                            </span>
                          </div>
                        </>
                      )}
                      ...
                      {prediction.predictions.readmission.socialDeterminants && (
                        <>
                          <div className="flex items-center justify-between">
                            ...
                            <Badge variant={prediction.predictions.readmission.socialDeterminants.livesAlone ? 'destructive' : 'default'}>
                              {prediction.predictions.readmission.socialDeterminants.livesAlone ? 'Sí' : 'No'}
                            </Badge>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-sm">Tiene cuidador</span>
                            <Badge variant={prediction.predictions.readmission.socialDeterminants.hasCaregiver ? 'default' : 'destructive'}>
                              {prediction.predictions.readmission.socialDeterminants.hasCaregiver ? 'Sí' : 'No'}
                            </Badge>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-sm">Acceso a transporte</span>
                            <Badge variant={prediction.predictions.readmission.socialDeterminants.transportationAccess ? 'default' : 'destructive'}>
                              {prediction.predictions.readmission.socialDeterminants.transportationAccess ? 'Sí' : 'No'}
                            </Badge>
                          </div>
                        </>
                      )}
                  ...
                  <CardContent>
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span>{prediction.metadata.accuracy.historical ? `${prediction.metadata.accuracy.historical * 100}%` : 'N/A'}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
      ...
      {hasCriticalAlerts() && (
        <Alert variant="destructive" className="animate-pulse">
          ...
          <AlertDescription>
            Hay alertas críticas que requieren su atención inmediata. 
            Revise la pestaña de alertas para más detalles.
          </AlertDescription>
        </Alert>
      )}
    </div>
